...
import { mergeGroupDataOnFlush } from "./dataMerge.js";
...
export function makeSupabaseStore(supabase, groupId, { debounceMs = 700, onFlush, onStale, onStatus } = {}) {
  ...
  // Keep requests on disk until the server acknowledges them, including during pagehide.
  const persistPending = () => writePersistedPending(groupId, { ...writing, ...pending });
  ...
  async function writePending() {
    ...
    try {
      ...
      /* data = בלוב מלא. מטמון ישן + סימון שולם היה דורס ערבים חדשים בשרת.
         לפני כתיבה — מאחדים sessions עם השרת (לא מוחקים id שקיים רק שם). */
      let adoptedRemoteData = false;
      if ("data" in patch && patch.data != null && typeof patch.data === "object") {
        const { data, error } = await supabase
          .from("groups")
          .select("data")
          .eq("id", groupId)
        ...
        if (data?.data && typeof data.data === "object") {
          const merged = mergeGroupDataOnFlush(patch.data, data.data);
          patch.data = merged.data;
          if (merged.adopted) {
            adoptedRemoteData = true;
            console.warn(
              "[store] rescued sessions from server on flush:",
              merged.rescuedIds.join(", ")
            );
          }
          cache[DB_KEY] = JSON.stringify(patch.data);
          lsSet(lsKey(groupId, "data"), cache[DB_KEY]);
        }
      }
      ...
      if (typeof onFlush === "function") {
        onFlush(patch, { adoptedRemoteLive, adoptedRemoteData });
      }
      return true;
    } catch (error) {
