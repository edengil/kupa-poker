...
export function buildPeriodReport(data, rep) {
  ...
  const winners = sorted.filter((t) => t.amount > 0);
  const even = sorted.filter((t) => t.amount === 0);
  const debtors = sorted.filter((t) => t.amount < 0).sort((a, b) => a.amount - b.amount);
  const medals = ["🥇", "🥈", "🥉"];
  const winLines = winners.map((t, i) => {
    const medal = i < 3 ? medals[i] : "•";
    return `${medal} ${t.name} מגיע ${fmtSigned(t.amount)}₪`;
  });
  const evenLines = even.map((t) => `• ${t.name} סגר באפס`);
  const debtLines = debtors.map((t) => `💸 ${t.name} חייב ${fmtSigned(t.amount)}₪`);

  const most = Object.entries(nightsPer).sort((a, b) => b[1] - a[1])[0];
  const topTipper = Object.entries(tipChipsPer).sort((a, b) => b[1] - a[1])[0];
  const winner = winners[0];
  const loser = debtors[0];
  ...
  if (evenLines.length) {
    out.push("", "✅ סגרו באפס", ...evenLines);
  }
  if (debtLines.length) {
    out.push("", "📉 מי חייב", ...debtLines);
  }
