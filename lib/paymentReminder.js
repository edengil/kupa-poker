...
/** האם עכשיו חלון השליחה (10:00 ישראל), אלא אם force. */
...
/** טקסט לוואטסאפ — לינק לאפליקציה לסימון שולם. */
export function buildPaymentReminderText(session, unpaid, { siteUrl, slug } = {}) {
  const dateLabel =
    session?.d != null && session?.mo != null
      ? `${session.d}.${session.mo}`
      : String(session?.iso || "");
  const lines = (unpaid || []).slice(0, 12).map((t) => `• ${t.from} → ${t.to}: ${t.amount}₪`);
  const extra =
    (unpaid || []).length > 12 ? `… ועוד ${(unpaid || []).length - 12}` : null;
