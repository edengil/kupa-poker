...
/** מצב אחד לשורה: ממתין, או סגירה עם הפעולה ומי שביצע אותה. בלי שם אם אין רישום. */
export function confirmationStatusText(row) {
  if (!row?.closed) return "ממתין";
  if (row.action === "paid") return row.by ? `שולם · ${row.by}` : "שולם";
  if (row.action === "received") return row.by ? `התקבל · ${row.by}` : "התקבל";
  return "נסגר";
}
