/* מתי לבקש אישור להתראות מחוץ לאפליקציה.
   הפתיחה הראשונה רק נרשמת. הבקשה מופיעה בפתיחה הבאה, פעם אחת. */
...
export function nextPushPrompt(prev, { permission = "default", support = "supported", visitId }) {
  ...
  if (support === "needs-install") {
    if (state.installHint) return { ...base, show: false, mode: null };
    return { ...base, installHint: true, show: true, mode: "install" };
  }
