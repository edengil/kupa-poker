...
/** צד הבעלים: ערוץ ששולח ping אחרי כל כתיבה שנחתה במסד.
    onPing מאזין לפינגים שמגיעים מבחוץ (הבוט בוואטסאפ כותב דרך השרת ומשדר) —
    ההודעות של הערוץ עצמו לא חוזרות אליו, כך שאין הד לכתיבות מקומיות.
    חשוב שההאזנה תשב על אותו ערוץ: פתיחת ערוץ שני לאותו topic מפילה את
    ספריית ה-realtime. */
export function createBroadcaster(supabase, slug, onPing) {
  const channel = supabase.channel(channelName(slug));
  if (typeof onPing === "function") {
    channel.on("broadcast", { event: "update" }, () => onPing());
  }
  let ready = false;
...
export async function fetchSnapshot(supabase, slug) {
  const { data, error } = await supabase.rpc("public_group_v2", { p_slug: slug });
  if (error) {
    console.error("Refresh failed:", error.message);
    return null;
  }
  const row = Array.isArray(data) ? data[0] : data;
  return row ? { id: row.id, name: row.name, data: row.data, live: row.live, config: row.config, updatedAt: row.updated_at } : null;
}
