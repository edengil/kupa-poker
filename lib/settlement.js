/* ============================================================================
   ...
   הסדר:
     ...
     4. זוגות ששניהם עדיין חייבים — משלמים יחד ("דור ועדן לירז מעבירים…")
     5. אחר כך התאמות עם העדפות:
        - נתנאל לא אוהב פייבוקס → מעביר לעדן/אורן רק אם אין ברירה
...
export function settle(players = [], cps = CPS) {
  ...
  const moveJoint = (i, j, to, amount) => {
    ...
    const takeI = Math.min(debtI, left);
    bal[i] += takeI;
    left -= takeI;
    const takeJ = Math.min(debtJ, left);
    bal[j] += takeJ;
    bal[to] -= amount;
    transfers.push({
  ...
  // 2) זוג ששניהם עדיין חייבים — משלמים יחד למרוויחים
  for (const [a, b] of COUPLES) {
    const i = findByCoupleName(names, a);
    const j = findByCoupleName(names, b);
    if (i < 0 || j < 0) continue;
    for (;;) {
      ...
      // בחירת מרוויח — מעדיפים לא־פייבוקס־בלבד אם אפשר
      let winner = -1;
      let winnerCash = -1;
      bal.forEach((v, k) => {
        ...
        } else if (winner === -1 || v > bal[winner]) winner = k;
      });
      const w = winner !== -1 ? winner : winnerCash;
      if (w === -1) break;
      moveJoint(i, j, w, Math.min(combined, bal[w]));
    }
  }

  // 3) שאר ההעברות — עם העדפת נתנאל (לא לפייבוקס אלא אם אין ברירה)
  const findBestPair = () => {
    const losers = [];
    const winners = [];
    bal.forEach((v, i) => {
