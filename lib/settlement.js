/* ============================================================================
   ...
   הסדר כאן לא שרירותי:
     ...
     3. זוגות קבועים מכסים אחד את השני קודם (עדן↔אורן, דור↔עדן לירז)
     4. אחר כך המפסיד הגדול מעביר למרוויח הגדול, וחוזר חלילה

   הכיסוי בין בני הזוג קורה לפני שאר השולחן: אם עדן גיל הפסיד ואורן
   גיל הרוויחה — עדן מעביר לאורן קודם, ורק את מה שנשאר ממשיכים לחלק.
   ============================================================================ */
...
/* זוגות שמכסים אחד את השני לפני שאר השולחן */
export const COUPLES = [
  ["עדן גיל", "אורן גיל"],
  ["דור לירז", "עדן לירז"],
];

/* כינויים קצרים → השם המלא, כדי שהזוגות יזוהו גם כשרשמו רק "עדן"/"אורן"/"דור" */
...
export function settle(players = [], cps = CPS) {
  ...
  const bal = [...adjusted];
  const transfers = [];
  const move = (from, to, amount, pair = false) => {
    if (amount <= 0) return;
    const row = { from: names[from], to: names[to], amount };
    if (pair) row.pair = true;
    transfers.push(row);
    bal[from] += amount;
    bal[to] -= amount;
  };

  // זוגות קודם — מי שמפסיד מעביר לבן/בת הזוג שמרוויח/ה, לפני כולם
  for (const [a, b] of COUPLES) {
    const i = findByCoupleName(names, a);
    const j = findByCoupleName(names, b);
    if (i < 0 || j < 0) continue;
    if (bal[i] < 0 && bal[j] > 0) move(i, j, Math.min(-bal[i], bal[j]), true);
    else if (bal[j] < 0 && bal[i] > 0) move(j, i, Math.min(-bal[j], bal[i]), true);
  }
