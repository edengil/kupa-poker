...
import { nextTipCompliment } from "./tipCompliments";
...
/* טיפ מג'יטונים של השחקן (לא מהקופה): "אופיר טיפ 10" / "אופיר נתן טיפ 10" */
...
export function applyCommands(live, cmds, msgId, cps = CPS) {
  ...
  for (let cmd of cmds) {
    ...
    if (cmd.kind === "tip") {
      ...
      const { compliment, bag, last } = nextTipCompliment(state);
      ...
      acks.push(`${p.name} נתן טיפ ${amount} ג' · ${compliment} 👏`);
      continue;
    }
...
/* -------------------------- חשבון סופי --------------------------
   ...
   בסוף: מי לא נתן טיפ; ואם גם הרוויח — נזיפה קלה. */
function settleSummary(state, cps = CPS) {
  ...
  const tipped = new Set(
    (state.tips || [])
      ...
      .map((t) => t.name)
      .concat(state.players.filter((p) => (+p.tipsGiven || 0) > 0).map((p) => p.name))
  );
  const noTip = state.players.filter((p) => !tipped.has(p.name));
  if (!noTip.length) return body;

  const lines = [`🙊 בלי טיפ הערב: ${noTip.map((p) => p.name).join(", ")}`];
  const wonNoTip = noTip.filter((p) => {
    const row = withNet.find((x) => x.p.name === p.name);
