...
function parseLine(text, defaultAmt, gameActive) {
  ...
  if (gameActive) {
    ...
    if (m) return { kind: "add", name: m[2].trim(), amount: +m[1] };
    // שם בלבד — נבדק אחרון, ומטופל רק כשסגירה מודרכת פעילה
    if (RE_NAME_ONLY.test(line)) return { kind: "nameOnly", name: line };
  }
  return null;
}
...
export function helpText() {
  return [
    ...
    "מספר הכניסות שהוכנו",
    "  כניסות 30",
    "  עוד 5 כניסות     (מוסיף למלאי הקיים)",
    "",
    ...
    "",
    "סיום הערב",
    "  סיום משחק     סגירה מודרכת: שם ← ג'יטונים ← הבא,",
    "                ובסוף חשבון סופי אוטומטי",
    "  בטל סיום      יציאה מהסגירה המודרכת",
    "",
    "מידע",
    ...
    "הסדר גמיש: גם \"50 דן\", \"20 כניסות\" ו\"דן הסר\" עובדים.",
  ].join("\n");
}
