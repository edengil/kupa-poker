...
/* -------------------------- חשבון סופי --------------------------
   ...
   (הגדול קודם) — עם שורת רווח בין הקבוצות.
   בסוף בלוק טיפים: שבח למי שנתן (מקום 1+2), צחוק על מי שלא. */
function settleSummary(state, cps = CPS) {
  ...
  const tipLines = tipSummaryBlock(state, withNet);
  if (!tipLines) return body;
  return `${body}\n\n${tipLines}`;
}

/** סיכום טיפים לסוף ערב — תמיד כשהיו שחקנים (גם אם כולם טיפאו). */
function tipSummaryBlock(state, withNet) {
  ...
  if (tippers.length) {
    ...
    } else if (tippers.length === 2) {
      /* כבר יש מקום 1+2 */
    } else if (tippers.length === 1) {
      /* רק ראשון */
    }
  } else {
