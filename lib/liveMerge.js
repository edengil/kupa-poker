...
/**
 * הטאב המקומי מפגר אחרי השרת: קנייה נמוכה יותר או טיפ מצטבר נמוך יותר.
...
export function mergeLiveStates(local, remote, aliases = DEFAULT_ALIASES) {
  ...
  const order = (
    takeRemoteCashout && remoteC.order.length
      ? remoteC.order
      : localC.order.length
        ? localC.order
        : remoteC.order
  ...
  for (const key of keys) {
    ...
    if (lp && rp) {
      ...
      mergedPlayers.push({ ...lp, name: key });
    } else if (rp && botAhead) {
      /* שחקן חדש מהבוט בלבד — לא להחזיר שחקן שנמחק בכוונה במסך. */
  ...
  return {
    ...
    actionLog: pickActionLog(local.actionLog, remote.actionLog, takeRemoteCashout),
    entriesCount:
...
export function decideLivePoll(localSnap, remote, lastFp) {
  ...
  /* שינוי מקומי (הסרת שחקן, יומן, יצא) — דוחפים לשרת בלי remount שגורם לקפיצות. */
  return { action: "flush" };
}
